...
run: build
	$(OPAM_EXEC) $(DUNE) exec -- snkal

debug: build
	$(OPAM_EXEC) ocamldebug _build/default/snkal/main.bc

DOCS_PATH=docs/
DOCS_NAME=snkal
DOCS_DESCR=Learning symbolic NetKAT automata from simulated firewalls using the L* algorithm
...
<meta content="https://github.com/CharlesAverill/snkal" property="og:url" />
endef
...
docs: cleandocs build
	...
	mv $(DOCS_PATH)snkal/snkal.html $(DOCS_PATH)index.html
	mv $(DOCS_PATH)snkal $(DOCS_PATH)module
